...
export async function handleSale(
  ...
  if (!spreadsheetId || !accessToken) {
    return showErrorPopup({
      ...
      message: 'Spreadsheet ID or access token is missing.',
    });
  }
  ...
  try {
    ...
    const inventoryRows = await getSheetData(
      spreadsheetId,
      accessToken,
      'Inventory',
    );
    ...
    if (rowIndex === -1)
      return showErrorPopup({
        ...
        message: `The product "${productName}" does not exist in inventory.`,
      });

    const oldQty =
      editRowIndex !== undefined
        ? await handleEditSaleMark(spreadsheetId, accessToken, editRowIndex)
        : 0;
    ...
    if (adjustedStock < 0)
      return showErrorPopup({
        ...
        message: `Only ${parseIntSafe(
          inventoryRows[rowIndex][1],
        )} units available in stock for "${productName}".`,
      });
    ...
    const success = await appendData(
      spreadsheetId,
      accessToken,
      'Sales',
      rowValues,
    ...
    showSuccessPopup(
      editRowIndex !== undefined ? 'Sale updated!' : 'Sale saved!',
    );
    setShowModal?.(false);
    fetchSheetData?.();
  } catch (error) {
...
function findInventoryRow(
  ...
  return inventoryRows.findIndex(
    row =>
      normalizeString(row[0]) === normalizeString(productName) &&
      (row[6] || '').toLowerCase() === 'false',
  );
}

async function handleEditSaleMark(
  ...
  if (!salesData) {
    showErrorPopup({
      title: 'Error',
      message: 'Failed to load sale data for editing.',
    });
    return 0;
  }
...
async function getNextTransactionId(
  ...
  const salesData =
    (await getSheetData(spreadsheetId, accessToken, 'Sales')) || [];
...
async function updateInventoryStock(
  ...
  await axiosInstance.put(
    `/${spreadsheetId}/values/Inventory!B${rowIndex + HEADER_OFFSET}:C${
